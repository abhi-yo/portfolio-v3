...
const MacOSDock: React.FC<MacOSDockProps> = ({ 
  ...
  const getResponsiveConfig = useCallback(() => {
    if (typeof window === 'undefined') {
      return { baseIconSize: 20, maxScale: 1.2, effectWidth: 120 };
    }

    // Base calculations on smaller dimension for better mobile experience
    const smallerDimension = Math.min(window.innerWidth, window.innerHeight);
    
    // Scale icon size based on screen size - making everything much smaller
  ...
  const handleAppClick = (appId: string, index: number) => {
    if (iconRefs.current[index]) {
      const windowWithGsap = window as unknown as { gsap?: { to: (target: HTMLElement | null, options: object) => void } };
      if (typeof window !== 'undefined' && windowWithGsap.gsap) {
        ...
        windowWithGsap.gsap.to(iconRefs.current[index], {
          y: bounceHeight,
          duration: 0.2,
          ease: 'power2.out',
          yoyo: true,
  ...
  const padding = Math.max(8, baseIconSize * 0.12);
    ...
    <div 
      ...
      style={{
        ...
        borderRadius: `${Math.max(6, baseIconSize * 0.25)}px`,
        border: '1px solid rgba(255, 255, 255, 0.05)',
        boxShadow: `
          0 ${Math.max(1, baseIconSize * 0.02)}px ${Math.max(4, baseIconSize * 0.15)}px rgba(0, 0, 0, 0.2),
          inset 0 1px 0 rgba(255, 255, 255, 0.05)
        `,
        padding: `${Math.max(2, padding * 0.4)}px`
      }}
